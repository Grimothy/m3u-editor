...
it('filters filmography to series+vod in the originating playlist', function () {
    // The page-level TMDB mock returns 2 credits: tv (id 1399, GoT) + movie (id 550, Fight Club).
    // The fake above already covers person + combined credits; add a person/combined fake
    // with media types we can filter.
    Cache::flush();
    Http::preventStrayRequests();
    Http::fake([
        'api.themoviedb.org/3/person/*' => Http::sequence()
    ...
    // Other two credits should be filtered out.
...
it('does not filter when playlistId is zero', function () {
    // Default fake (person + combined credits) gives 2 unfiltered items.
    Cache::flush();
    $user = User::factory()->create();
    $this->actingAs($user);
...
it('does not filter when playlistId belongs to another user (non-admin)', function () {
    ...
    // playlistId via query string and inspect which of their Series/Vod
    // exist. The trait's filmographyPlaylistId() impl should return 0 in that
    // case and the filter is skipped (safe fallback to global TMDB).
    Cache::flush();
    Http::preventStrayRequests();
...
it('redirects to local Series when openFilmographyItem is called inside a playlist-scoped page', function () {
    ...
    // Calling openFilmographyItem triggers redirect; assert the URL it would navigate to.
