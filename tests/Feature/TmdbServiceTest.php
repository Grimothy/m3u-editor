...
it('returns tv cast with TMDB personId for click-through to filmography', function () {
    Cache::flush();
    Http::fake([
        'https://api.themoviedb.org/3/tv/*/credits*' => Http::response([
            'cast' => [
                [
                    'id' => 1223786,
                    'name' => 'Emilia Clarke',
                    'character' => 'Daenerys Targaryen',
                    'profile_path' => '/r9Oq6JeKQq6q3q3q3q3q3q3q3q3.jpg',
                ],
                [
                    'id' => 229719,
                    'name' => 'Kit Harington',
                    'character' => 'Jon Snow',
                    'profile_path' => '/4Mq9jb7wYwgn5a0xMzMzwOuVNtj.jpg',
                ],
            ],
        ], 200),
    ]);
...
it('returns ALL of an actor combined credits with no implicit cap', function () {
    // Regression: the filmography page must not silently drop credits. Some actors
    // (e.g. prolific character actors) have >100 entries on TMDB. The page should
    // surface every one — only the originating playlist scope filters them.
    Cache::flush();
    $cast = [];
    for ($i = 1; $i <= 50; $i++) {
        $cast[] = [
            ...
            // Movies use title, TV uses name — alternate to cover both branches
            ($i % 2 ? 'title' : 'name') => "Credit {$i}",
            'character' => "Character {$i}",
            ($i % 2 ? 'release_date' : 'first_air_date') => sprintf('%04d-01-01', 2000 + ($i % 25)),
            'poster_path' => "/p{$i}.jpg",
        ];
    }
    Http::fake([
        'api.themoviedb.org/3/person/*/combined_credits*' => Http::response([
            'cast' => $cast,
        ], 200),
    ]);

    $service = new TmdbService($this->settings);
    $credits = $service->getPersonCombinedCredits(123);

    // All 50 returned (no implicit take/limit) and every tmdb_id round-trips.
    expect($credits)->toHaveCount(50)
        ->and(collect($credits)->pluck('tmdb_id')->map(fn ($id) => (int) $id)->sort()->values()->all())
        ->toEqual(range(1001, 1050));
});
