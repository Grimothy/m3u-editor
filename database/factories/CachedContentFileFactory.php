...
class CachedContentFileFactory extends Factory
{
    ...
    public function definition(): array
    {
        return [
            ...
            'season_number' => null,
            'episode_number' => null,
            // PR B: default quality to null so the model's auto-built
            // fingerprint matches CachedContentDispatchService's fingerprint
            // (which has no quality input - Phase 1's standalone cache has
            // no per-rule quality preference yet; Phase 2's DG-aware
            // dispatcher adds it back).
            'quality' => null,
            'disk' => null,
            'file_path' => null,
    ...
    public function completed(): static
    {
        return $this->state(fn (array $attributes) => [
            ...
            // PR B: completed cache files land on the new `cache` disk
            // (config/filesystems.php) - the standalone cache is the only
            // place DownloadCachedContentFile::handle() writes its bytes,
            // and CachedContentRetentionService::deleteIds() uses the
            // resolved disk for on-disk cleanup.
            'disk' => 'cache',
