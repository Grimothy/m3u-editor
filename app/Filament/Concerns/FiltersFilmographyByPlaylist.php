...
trait FiltersFilmographyByPlaylist
{
    ...
    protected function filterFilmographyToPlaylist(array $filmography): array
    {
        ...
        // Build hash sets via cursor() so memory stays flat regardless of playlist
        // size. Plain closures (not arrow functions) — PHP arrow fns capture by
        // value, so $set[...] = true inside `fn (...) => ...` silently no-ops
        // instead of mutating the outer array.
        $seriesTmdbSet = [];
        Series::query()
