    ...
    <div>
        ...
        <div x-show="loading" class="flex items-center justify-center p-8">
            <div class="flex items-center space-x-2">
                <svg class="animate-spin h-5 w-5 text-indigo-500 dark:text-indigo-400" xmlns="http://www.w3.org/2000/svg"
        ...
        <div x-show="!loading && !error" class="space-y-6" wire:ignore.self>
            ...
            <div
                class="bg-white ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10 dark:bg-gray-900 rounded-md p-3">
                <div class="flex flex-col gap-4">
                    <!-- Header Row -->
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <!-- Date Navigation -->
                        <div class="flex items-center justify-between sm:justify-start gap-2 sm:gap-4">
                            <x-filament::button icon="heroicon-m-chevron-left" icon-position="before" color="gray"
                                size="sm" @click="previousDay()">
                            ...
                            <div class="flex flex-col text-center sm:text-left">
                                <h3 class="text-base sm:text-lg font-medium text-gray-900 dark:text-gray-100 truncate"
                                    x-text="epgData?.epg?.name || epgData?.playlist?.name || 'EPG Viewer'"></h3>
                                <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400"
                                    x-text="formatDate(currentDate)"></p>
                            </div>

                            <x-filament::button icon="heroicon-m-chevron-right" icon-position="after" color="gray"
                                size="sm" @click="nextDay()">
                        ...
                        <div class="flex items-center justify-center sm:justify-end gap-2">
                            <x-filament::button icon="heroicon-m-calendar" icon-position="before" color="gray" size="sm"
                                x-show="!isToday()" @click="goToToday()">
                                <span class="hidden sm:inline">Today</span>
                                <span class="sm:hidden">Today</span>
                            </x-filament::button>
                            <x-filament::button icon="heroicon-m-clock" icon-position="before" color="gray" size="sm"
                                x-show="isToday()" @click="scrollToCurrentTime()">
                        ...
                        <div class="relative flex-1">
                            <x-filament::input.wrapper>
                                <x-filament::input type="text" x-model="searchTerm"
                                    @keydown="handleSearchKeydown($event)" placeholder="Search channels..." />
                                <x-slot name="suffix">
                                    <!-- Clear Button -->
                                    ...
                                    <!-- Search Button -->
                                    <button @click="performSearch()" :disabled="!searchTerm.trim()"
                                        ...
                                        class="p-1 transition-colors" title="Search">
            ...
            <div class="bg-white ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10 dark:bg-gray-900 rounded-md p-0 overflow-hidden relative"
                ...
                <div x-show="loadingMore" x-transition:enter="transition ease-out duration-300"
                    x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                    x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100"
                    x-transition:leave-end="opacity-0" wire:ignore
                ...
                <!-- Time Header -->
                <div
                    class="sticky top-0 z-10 bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
                    <div class="flex">
                        <!-- Channel Column Header -->
                        <div :class="isMobile ? 'w-32' : 'w-60'"
                            class="px-2 md:px-4 py-3 border-r border-gray-200 dark:border-gray-600 bg-gray-100 dark:bg-gray-800">
                            <div class="flex items-center justify-between">
                                ...
                                <div x-show="isSearchActive && !isMobile" class="flex items-center space-x-1">
                                    <x-heroicon-m-magnifying-glass
                                        class="w-3 h-3 text-indigo-500 dark:text-indigo-400" />
                        ...
                        <div class="flex-1 relative overflow-hidden">
                            <div class="overflow-x-auto time-header-scroll"
                                ...
                                <div class="flex relative" style="width: 2400px;"> <!-- 24 hours * 100px per hour -->
                                    <template x-for="hour in timeSlots" :key="hour">
                                        <div class="px-1 md:px-2 py-3 border-r border-gray-200 dark:border-gray-600 text-center bg-gray-100 dark:bg-gray-800"
                                            style="width: 100px;">
                                            <span class="font-medium text-xs text-gray-700 dark:text-gray-300"
                                                x-text="formatTime(hour)"></span>
                                        </div>
                                    </template>
                                    <!-- Current time indicator (moves with content) -->
                                    <div x-show="isToday() && currentTimePosition >= 0"
                ...
                <div class="flex h-full overflow-hidden pb-[2.8rem]" x-data="{
                    virtualScrollTop: 0,
                    ...
                    <div :class="isMobile ? 'w-32' : 'w-60'"
                        class="border-r border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 overflow-hidden">
                        <div class="overflow-y-auto overflow-x-hidden h-full" @scroll="
                            ...
                            <div class="relative" :style="`height: ${totalChannels * itemHeight}px;`">
                                ...
                                <template x-for="item in visibleChannels" :key="item.id">
                                    <div :class="isMobile ? 'px-2 py-2' : 'px-4 py-3'"
                                        ...
                                        <div class="flex-shrink-0">
                                            <img :src="item.channel.icon || '/placeholder.png'"
                                                :alt="item.channel.display_name"
                                                :class="isMobile ? 'w-6 h-6' : 'w-8 h-8'" class="rounded object-contain"
                                                onerror="this.src='/placeholder.png'">
                                        </div>
                                        <div class="min-w-0 flex-1">
                                            <p :class="isMobile ? 'text-xs' : 'text-sm'"
                                                class="font-medium text-gray-900 dark:text-gray-100 truncate"
                                                x-text="item.channel.display_name"
                                                x-tooltip="item.channel.display_name"></p>
                                            <p x-show="!isMobile"
                            ...
                            <div x-show="isSearchActive && channelOrder.length === 0 && !loadingMore && !loading"
                                ...
                                <div class="flex flex-col items-center space-y-2">
                                    <x-heroicon-m-magnifying-glass :class="isMobile ? 'w-6 h-6' : 'w-8 h-8'"
                                        class="text-gray-400 dark:text-gray-500" />
                                    <div :class="isMobile ? 'text-xs' : 'text-sm'"
                                        class="font-medium text-gray-600 dark:text-gray-400">No channels found</div>
                                    <div class="text-xs text-gray-500 dark:text-gray-400"
                                        x-text="'No results for &quot;' + searchTerm + '&quot;'"></div>
                    ...
                    <div class="flex-1 overflow-auto relative timeline-scroll" @scroll="
                        ...
                        <div class="relative overflow-hidden" style="width: 2400px;"> <!-- 24 hours * 100px per hour -->
                            ...
                            <div class="relative" :style="`height: ${totalChannels * itemHeight}px;`">
                                <template x-for="item in visibleChannels" :key="item.id">
                                    <div class="absolute w-full border-b border-gray-100 dark:border-gray-600"
                                        :style="`top: ${item.top}px; height: ${itemHeight}px;`">
